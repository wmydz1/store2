...
<div class="top-banner" id="top-banner-block">
    ...
    <?php $pid =1;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1508594400 and end_time > 1508594400 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
...
$("#header_cart_list > .micart > .les").hover(function(){	 
    ...
    <h3 class="h3-title">常见问题分类</h3>
        <?php
                                ...
                                $md5_key = md5("select * from `__PREFIX__article_cat` where cat_id in(1,2,3,4,7)");
                                $result_name = $sql_result_v = S("sql_".$md5_key);
                                if(empty($sql_result_v))
                                {                            
                                    $result_name = $sql_result_v = \think\Db::query("select * from `__PREFIX__article_cat` where cat_id in(1,2,3,4,7)"); 
                                    S("sql_".$md5_key,$sql_result_v,1);
                                }    
                              foreach($sql_result_v as $k=>$v): ?>
	         <dl class="subside-mod on">
	            <dt class="title">
	               	 <?php echo $v[cat_name]; ?><b class="icon02"></b>
	            </dt>           
	            <dd class="subside-cnt">
	                <ul class="subside-list" style="padding: 6px 10px 5px 10px;">
	                    <?php
	                        ...
	                        <li style="height: 20px;" data-parent-name="购物指南" data-parent-id="21" data-name="交易条款" data-id="28" class="list-item <?php if($article['article_id'] == $v2[article_id]): ?>current<?php endif; ?>">
	                            <a style="white-space: nowrap;text-overflow: ellipsis;overflow: hidden;display: block;" href="<?php echo U('Home/Article/detail',array('article_id'=>$v2[article_id])); ?>"><?php echo $v2[title]; ?></a>
	                        </li>
	                    <?php endforeach; ?>
	                </ul>
	            </dd>
    ...
    &gt;
    <a href="#"><span id="sLevel2"><?php echo $cat_name; ?></span></a>

</div>

<div class="contxt">
    <div class="help-tit1 flk06"><?php echo $article['title']; ?></div>
