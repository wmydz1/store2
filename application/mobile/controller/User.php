...
//        $result = $cartLogic->cartList($this->user, $this->session_id,1); // 获取购物车商品
        if(I('type') == ''){
            $where = " c2.uid = {$this->user_id} and unix_timestamp(now()) < c1.use_end_time and unix_timestamp(now()) > c1.use_start_time and c1.condition <= {$result['total_price']['total_fee']} ";
        }
        if(I('type') == '1'){
           $where = " c2.uid = {$this->user_id} and c1.use_end_time < unix_timestamp(now()) or c1.use_start_time > unix_timestamp(now()) or {$result['total_price']['total_fee']}  < c1.condition ";
        }
